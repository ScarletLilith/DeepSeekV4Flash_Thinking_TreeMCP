...
每道题设 **10 个客观检查点**，每个 1 分，共 10 分。评分方式：人工逐项对照答案，判定该检查点是否被充分覆盖——覆盖则得 1 分，否则 0 分。不依赖关键词匹配或自动化脚本。

评分人手持检查点清单，逐题阅读三组输出，在表格中打 ✓/✗。
...
| 维度 | 计算方式 |
|------|----------|
| **内容质量（满分 50）** | 5 道题 × 10 检查点/题 = 50 分（人工评分，每项 1 分）|
| **成本效率（参考指标）** | 从 API 响应的 `usage` 字段提取 token 数，代入定价表计算 |
| **响应速度（参考指标）** | 端到端响应时间 |
| **缓存利用率（参考指标）** | `prompt_cache_hit_tokens / (hit + miss)` 计算缓存命中率 |
