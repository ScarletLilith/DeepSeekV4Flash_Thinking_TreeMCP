...
我们设计了一个对照实验，对比 **3 种方案** 在 **5 道高难度工程问题**（分布式共识、服务网格、RTOS内核、列式存储引擎、多模态AI Agent框架）上的表现。

### 测试分组

| 组 | 模型 | API | 工具 |
|:---:|------|:---:|:----:|
| **A** | GLM-5.2 | SiliconFlow | 无 |
| **B** | DeepSeek-V4-Flash | DeepSeek 官方 | chat_agent + create_branch |
| **C** | DeepSeek-V4-Flash | DeepSeek 官方 | 无 |

### 核心结果

| 指标 | A (GLM-5.2) | B (DS+工具) | C (DS纯思考) |
|------|:----------:|:-----------:|:-----------:|
| **总输出量** | 38,888 字 | **169,484 字** 🏆 | 57,565 字 |
| **总耗时** | 705s | 1,516s | **239s** 🏆 |
| **总 Token 消耗** | 28,459 | 210,482 | 29,184 |
| **总成本** | ¥0.75 | ¥0.21 | **¥0.057** 🏆 |
| **单题平均输出** | 7,778 字 | **33,897 字 (4.4倍)** 🏆 | 11,513 字 |
| **工具调用** | 0 | **30 次** 🏆 | 0 |
| **缓存命中率** | 0% | **最高 68%** 🏆 | 0% |

### 关键发现

- **使用 MCP 工具后，DeepSeek-V4-Flash 输出了 4.4 倍更详细的工程方案**——包含完整的 Go 风格 Raft 共识实现、汇编级 RTOS 调度器代码、生产级服务网格配置
- **树形思维**（`create_branch`）让模型可以深入探索 5-6 层，为架构、实现、测试、验证分别创建子分支
- **成本对比**：GLM-5.2 比 DeepSeek-V4-Flash 纯思考模式贵 **13 倍**（¥0.75 vs ¥0.057）。开启工具后 DeepSeek 成本因深度探索升至 ¥0.21（token 消耗增加 3.7 倍），但仍比 GLM-5.2 **便宜 3.6 倍**。
- **缓存命中率**在多轮工具调用中达到 68%，通过 DeepSeek 的 prefix caching 大幅降低有效输入成本

> 完整实验数据：[`results/comparison/report.md`](results/comparison/report.md)
